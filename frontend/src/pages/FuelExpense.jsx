...
import { Fuel, DollarSign, Calendar, Flame, AlertCircle, Plus, Trash2 } from 'lucide-react';
...
export const FuelExpense = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addNotification(`New logistical expense logged: ${formData.expenseType} for vehicle ${v?.registrationNo || ''}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      addNotification(`Expense ledger cleared for: ${expenseToDelete.expenseType}`);
  ...
  const fuelCostsTotal = expenses.reduce((acc, curr) => acc + (curr.expenseType === 'Fuel Fill' ? curr.amount : 0), 0);
  const otherCostsTotal = expenses.reduce((acc, curr) => acc + (curr.expenseType !== 'Fuel Fill' ? curr.amount : 0), 0);
  const combinedCostsTotal = fuelCostsTotal + otherCostsTotal;

  // Pagination slice
  const paginatedExpenses = [...expenses].reverse().slice(
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex flex-col">
        <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Financial Accounts</p>
        <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Fuel logs & Operating Expenses</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-400 font-bold uppercase">Combined Fuel Expenses</span>
            <span className="text-2xl font-black text-slate-800">${fuelCostsTotal.toLocaleString()}</span>
            <span className="text-[10px] text-slate-400 mt-1">Total liters: {expenses.reduce((acc, c) => acc + c.liters, 0).toLocaleString()} Liters</span>
          </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-400 font-bold uppercase">Tolls, Fees & Permits</span>
            <span className="text-2xl font-black text-slate-800">${otherCostsTotal.toLocaleString()}</span>
            <span className="text-[10px] text-slate-400 mt-1">Total miscellaneous expense lines</span>
          </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-400 font-bold uppercase">Total Combined Outlay</span>
            <span className="text-2xl font-black text-slate-800">${combinedCostsTotal.toLocaleString()}</span>
            <span className="text-[10px] text-emerald-600 font-bold mt-1">Active Logistics Accounts</span>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-4 bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col gap-5">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
            ...
            <div className="flex flex-col">
              <h4 className="text-sm font-bold text-slate-800">Record Operational Expense</h4>
              <span className="text-xs text-slate-400">Log cargo run costs and fuel refills</span>
            </div>
          </div>
        ...
        <div className="xl:col-span-8 bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-4.5 border-b border-slate-100">
            <h4 className="text-sm font-bold text-slate-800">Operational Expenses Registry</h4>
            <p className="text-xs text-slate-400 mt-1">Listing of registered financial logs and route transactions</p>
          </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginatedExpenses.map((expense) => (
                      <tr key={expense.id} className="hover:bg-slate-50/40 transition-colors">
                        <td className="px-6 py-4 font-bold text-slate-800 tracking-tight">{getVehicleRegNo(expense.vehicleId)}</td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-bold border ${
                            expense.expenseType === 'Fuel Fill' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-slate-100 border-slate-200 text-slate-700'
                          }`}>
                            {expense.expenseType}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-slate-600">
                          {expense.liters > 0 ? `${expense.liters.toLocaleString()} L` : '--'}
                        </td>
