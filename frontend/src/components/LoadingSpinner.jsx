...
export const LoadingSpinner = ({ size = 'md', className = '' }) => {
  ...
  return (
    <div className={`flex items-center justify-center p-6 ${className}`}>
      <div className={`animate-spin rounded-full border-t-blue-600 border-slate-200 ${sizes[size]}`}></div>
    </div>
  );
};
...
export const TableSkeleton = ({ rows = 5, cols = 5 }) => {
  return (
    <div className="w-full border border-slate-100 rounded-2xl overflow-hidden bg-white shadow-sm animate-pulse">
      <div className="bg-slate-50 border-b border-slate-100 h-12 flex items-center px-6">
...
export const CardSkeleton = () => {
  return (
    <div className="border border-slate-100 rounded-2xl p-5 bg-white shadow-sm animate-pulse flex flex-col gap-3">
